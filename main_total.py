...
def eval(model, args, test_set, n_test_batches, save_result_path=None):
    model.eval()
    ...
    with torch.no_grad():
        ...
        for j in range(n_test_batches):
            # --- LẤY TEST WORDS, TWORDS VÀ INPUT ---
            if args.model in ['KGNN', 'ASGCN']:
                if args.gcn == 1:
                    test_words, test_twords, test_x, test_xt, test_y, test_pw, test_adj, test_mask, train_adj_know, train_mask_know, train_x_know = get_batch_input_inference(
                    ...
                    if torch.cuda.is_available():
                        ...
                        train_adj_know, train_mask_know, train_x_know = train_adj_know.cuda(), train_mask_know.cuda(), train_x_know.cuda()
                    logit = model(test_x, test_xt, test_pw, test_adj, test_mask, train_adj_know, train_mask_know, train_x_know)
                elif args.gcn == 2:
                    test_words, test_twords, test_x, test_xt, test_y, test_pw, test_adj, test_mask, test_graph = get_batch_input_inference(
                        dataset=test_set, bs=args.bs, args=args, idx=j)
                    ...
                    if torch.cuda.is_available():
                        test_x, test_xt, test_y, test_pw, test_adj, test_mask, test_graph = test_x.cuda(), test_xt.cuda(), test_y.cuda(), test_pw.cuda(), test_adj.cuda(), test_mask.cuda(), test_graph.cuda()
                    logit = model(test_x, test_xt, test_pw, test_adj, test_mask, graph_embedding=test_graph)
                else:
                    test_words, test_twords, test_x, test_xt, test_y, test_pw, test_adj, test_mask = get_batch_input_inference(
                        dataset=test_set, bs=args.bs, args=args, idx=j)
                    test_x, test_xt, test_y, test_pw, test_adj, test_mask = torch.from_numpy(test_x), torch.from_numpy(
                        test_xt), torch.from_numpy(test_y).long(), torch.from_numpy(test_pw), torch.from_numpy(test_adj), torch.from_numpy(test_mask)
                    if torch.cuda.is_available():
                ...
                test_words, test_twords, test_x, test_xt, test_tag, test_y, test_pw = get_batch_input_inference(
                    dataset=test_set, bs=args.bs, args=args, idx=j)
                test_x, test_xt, test_tag, test_y, test_pw = torch.from_numpy(test_x), torch.from_numpy(
                    test_xt), torch.from_numpy(test_tag), torch.from_numpy(test_y).long(), torch.from_numpy(test_pw)
                if torch.cuda.is_available():
                ...
                test_words, test_twords, test_x, test_xt, test_y, test_pw = get_batch_input_inference(
                    dataset=test_set, bs=args.bs, args=args, idx=j)
                test_x, test_xt, test_y, test_pw = torch.from_numpy(test_x), torch.from_numpy(
                    test_xt), torch.from_numpy(test_y).long(), torch.from_numpy(test_pw)
                if torch.cuda.is_available():
            ...
            preds = torch.argmax(logit, -1).cpu().tolist()
            trues = test_y.cpu().tolist()
            sentiment_map = {0: 'positive', 1: 'negative', 2: 'neutral'}
            for text, aspect, gold, pred in zip(test_words, test_twords, trues, preds):
                results.append({
                    "text": " ".join(text) if isinstance(text, list) else text,
                    "aspect": " ".join(aspect) if isinstance(aspect, list) else aspect,
                    "gold_label": sentiment_map[gold],
            ...
            corrects += (torch.max(logit, 1)[1].view(test_y.size()).data == test_y.data).sum()
    ...
    if save_result_path is not None:
        ...
        with open(save_result_path, "w", encoding="utf-8", newline='') as f:
            writer = csv.DictWriter(f, fieldnames=["text", "aspect", "gold_label", "pred_label"])
            writer.writeheader()
...
def inference(args, model_path=None):
    ...
    if args.is_bert == 1:
        dataset, _, _ = build_dataset(ds_name=args.ds_name, bs=args.bs, dim_w=args.dim_w, is_bert=True)
        args.sent_len = len(dataset[0][0]['wids'])
        args.target_len = len(dataset[0][0]['tids'])
        n_test_batches = math.ceil(len(dataset) / args.bs)
        _, test_set = dataset

        bert = BertModel.from_pretrained('./bert-base-uncased')
        model = GCAE_Bert(bert=bert).to(device)
        state_dict = torch.load(model_path, map_location=device)
        model.load_state_dict(state_dict)
        return eval_bert(model, args, test_set, n_test_batches)
    else:
        if args.model == 'KGNN':
            dataset, embeddings, graph_embeddings, n_train, n_test = build_dataset(args=args)
            args.graph_embeddings = graph_embeddings
        ...
        start_time = time.time()
        save_result_path = "test_results.csv"
        test_acc, test_f1, results = eval(model, args, test_set, n_test_batches, save_result_path=save_result_path)
        print(f"test_time: {time.time() - start_time:.4f}s")
        print(f"Đã lưu chi tiết dự đoán tại {save_result_path}")
...
if __name__ == '__main__':
    ...
    parser.add_argument("-dim_w", type=int, default=768)
    parser.add_argument("-dim_k", type=int, default=400)
    parser.add_argument("-is_test", type=int, default=0)
    parser.add_argument("-is_bert", type=int, default=2)
    ...
    if args.is_test == 1:
        ...
        acc, f1 = inference(args, test_path)
        ...
        print('Results:',
              f"best_acc: {max(accs)} best_f1: {max(f1s)} min_time: {min(times)}", 
              f"avg_acc: {sum(accs)/len(accs)} avg_f1: {sum(f1s)/len(f1s)} avg_time: {sum(times)/len(times)}")
