...
def get_rolled_and_unrolled_data(input_data, multi_hop,add_non_connect,max_hop):
    ...
    # Tập hợp các POS (Part of Speech) tags được chọn lọc thủ công
    # Mục đích: Xác định các từ loại thường chứa thông tin về cảm xúc/đánh giá trong ABSA (Aspect-Based Sentiment Analysis)
    # Các từ loại được chọn bao gồm:
    # - JJ, JJR, JJS: Tính từ (adjective) ở dạng thường, so sánh hơn, so sánh nhất
    #   Ví dụ: good (JJ), better (JJR), best (JJS)
    # - RB, RBR, RBS: Trạng từ (adverb) ở dạng thường, so sánh hơn, so sánh nhất
    #   Ví dụ: quickly (RB), more quickly (RBR), most quickly (RBS)
    # - VB, VBD, VBG, VBN, VBP, VBZ: Các dạng của động từ (verb)
    #   Ví dụ: 
    #   - VB: động từ nguyên thể (go)
    #   - VBD: động từ quá khứ (went)
    #   - VBG: động từ dạng -ing (going)
    #   - VBN: động từ phân từ quá khứ (gone)
    #   - VBP: động từ hiện tại số ít (goes)
    #   - VBZ: động từ hiện tại số nhiều (go)
    ...
    sentiments_lookup = {0: "positive", 1: "negative", 2: "neutral"}
